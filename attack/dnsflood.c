...
uint64_t g_seed = 0;
static inline uint64_t rand_by_seed(uint64_t seed)
{
    g_seed = (g_seed + seed) * 1103515245 + 12345;
    return g_seed;
}


static inline char rand_alpha(char c)
{
    return  'a' + (rand_by_seed(c) % 26);
}



static inline dns_name_rand(char * name)
{
    int i = 0;
